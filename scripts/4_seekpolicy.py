...
def generate_rag_summaries(
    ...
    intra_doc_parallelism: int = 1,  # parallel workers for table analysis + chunk blend loops
    checkpoint_path: (
        Path | None
    ) = None,  # per-doc resume: save each generated query immediately
) -> pd.DataFrame:
    ensure_openai_api_key_loaded()
    ...
    # ------------------------------------------------------------------
    # Per-doc checkpoint: load any previously saved results so already-
    # completed docs are skipped, and flush each new result immediately
    # so a crash only loses the current doc rather than the whole run.
    # Checkpoint file: one row per doc with columns [doc_id, target_col].
    # ------------------------------------------------------------------
    _ckpt_lock = threading.Lock()

    def _load_doc_checkpoint() -> dict[str, str]:
        """Return {doc_id: generated_text} from the checkpoint file, or {}."""
    ...
    def _append_doc_checkpoint(doc_id: str, text: str) -> None:
        """Append a single completed doc to the checkpoint CSV (thread-safe)."""
    ...
    # Pre-fill already-completed docs from checkpoint so they are skipped below.
    ...
    def analyse_one_title(
        ...
        """Run dataset load + LLM analysis for a single table title. Returns the step query string or None."""
        ...
        dataset = meta_obj.get("dataset", {}) if isinstance(meta_obj, dict) else {}
        description = ""
        if isinstance(dataset, dict):
            description = clean_text(dataset.get("description", ""))

        dataset_text_lines = [f"Title: {title}"]
        if description:
            dataset_text_lines.append(f"Description: {description}")
        ...
        if country and policy_year is not None:
            ...
            if country_data:
                dataset_text_lines.append(f"Focused Country Data: {country}")
                dataset_text_lines.append(country_data)
        if region and policy_year is not None:
            region_data = retrieve_data_as_string_for_window(
                dataset_frame, region, policy_year
            )
            if region_data:
                dataset_text_lines.append(f"{region} Data:")
                dataset_text_lines.append(region_data)
        if policy_year is not None:
            world_data = retrieve_data_as_string_for_window(
                dataset_frame, "World", policy_year
            )
            if world_data:
                dataset_text_lines.append("World Data:")
                dataset_text_lines.append(world_data)

        dataset_text = "\n".join(dataset_text_lines).strip()
        if not dataset_text:
            return None
    ...
    def blend_one_query(
        ...
        """Run chunk retrieval + LLM blend for a single step query. Returns blended text or None."""
        ...
        context = format_table_block("retrieved_related_chunks", related_chunks)
        source_text = format_table_block("time_series_policy_metadata", [step_query])
        prompt = (
            "Generate a single coherent paragraph based on the following time series analysis and related content. "
            "The content and key insights MUST come strictly from the Time Series Analysis Chunk only. "
            "The Other Related Chunks are provided for writing style reference only — use them to match tone, "
            "sentence structure, and phrasing, but do NOT draw any facts, figures, or claims from them. "
            "Keep the paragraph relevant to the following policy context: "
            f"Sector: {sector}, Keywords: {keyword}, Topics: {topic}.\n"
            "Do not mention the name of any policy, plan, act, "
            "or refer to 'time series analysis' explicitly.\n\n"
            f"Time Series Analysis Chunk:\n{source_text}\n\n"
            f"Other Related Chunks (style reference only):\n{context}"
        )
        try:
            if debug_mode:
                debug_log_prompt("chunk_blend", prompt)
            new_chunk = call_openai_with_guard(llm.invoke, prompt)
    ...
    for idx in out.index:
        ...
        if intra_workers > 1 and len(selected_titles) > 1:
            with concurrent.futures.ThreadPoolExecutor(
                ...
                # preserve original title order for determinism
        ...
        final_prompt = (
            "You are a policy analyzer expert.\n"
            f"You are summarizing information related to a {sector} policy involving: {keyword}.\n"
            "Summarize the following chunks into one focused paragraph that captures "
            "the key trends and conditions relevant to this policy domain:\n\n"
...
#     if verbose_progress:
#         print(
#             f"[GEN][START] {progress_prefix} docs={total_docs} history_window={history_window}",
#             flush=True,
#         )
...
#                     debug_log_response(f"dataset_analysis::{title}", step_query)
#                 if step_query:
#                     step_1_queries.append(step_query)
#             except Exception:
#                 continue
...
#             context = format_table_block("retrieved_related_chunks", related_chunks)
#             source_text = format_table_block(
#                 "time_series_policy_metadata", [step_query]
#             )
#             prompt = (
#                 "Generate a single coherent paragraph based on the following time series analysis and related content. "
#                 "The content and key insights MUST come strictly from the Time Series Analysis Chunk only. "
...
#                     debug_log_response("chunk_blend", new_text)
#                 if new_text:
#                     all_new_chunks.append(new_text)
#             except Exception:
#                 continue
...
def main() -> None:
    ...
    # Create checkpoint directory up front so worker threads can write to it immediately.
